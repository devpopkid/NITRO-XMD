...
async function updateBio(Matrix) {
    try {
        ...
        const bio = `🧋ᴘᴏᴘᴋɪᴅ xᴍᴅ ɪs ᴀᴄᴛɪᴠᴇ🧋ᴀᴛ ${now.format('HH:mm:ss')} | ${lifeQuotes[Math.floor(Math.random() * lifeQuotes.length)]}`;
...
async function updateLiveBio(Matrix) {
    try {
        const now = moment().tz('Africa/Nairobi');
        await Matrix.updateProfileStatus(`🧋ᴘᴏᴘᴋɪᴅ xᴍᴅ ɪs ᴀᴄᴛɪᴠᴇ🧋ᴀᴛ ${now.format('HH:mm:ss')}`);
    } catch (err) {
